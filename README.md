...
Regarding the context rule described by Yannis Kassios [[1]] Formal Proof, A. N. Prior authored `The Runabout Inference-Ticket` [[2]], saying that `A. And (B)` is not equivalent to `A. Tonk (B)`. Later, Nuel D. Belnap authored `Tonk, Plonk and Plink` [[3]], saying that both `A,B` are deducible from `A. And (B)` in synthetic mode of logic relying on context.
